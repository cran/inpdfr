...
getSummaryStatsOCCUR<-function(wordF,getPlot=TRUE,mwidth=800,mheight=800){
  ...
  dfTableP<-data.frame(numW=tableP2,breaks=cut(as.numeric(names(tableP2)),breaks=seq(0,1,0.1)))
  aggregDfTableP<-stats::aggregate(dfTableP[,2] ~ dfTableP[,3],FUN=sum)

  if(getPlot==TRUE){
    grDevices::png(filename=paste0("RESULTS/PLOT_occurrence",".png"), width = mwidth, height = mheight)
      graphics::par(mar=c(5,4,2,2))
      try(graphics::plot(aggregDfTableP,las=2,log="y",ylab="Number of words",xlab=""),silent=TRUE)
    grDevices::dev.off()
  }
