...
switchOnDialogWait<-function(){
  dialogX<-gtkMessageDialog(parent = NULL, flags = 0, type = "info", buttons = "none", "Processing...",show=FALSE)
...
askQuit<-function(myobject) {
  gSignalConnect(myobject, "delete-event", function(event, ...) {
    dialog <- gtkMessageDialog(parent = myobject, flags = 0, type = "question", buttons = "yes-no", "Are you sure you want to quit?")
    out <- dialog$run()
    dialog$destroy()
...
open_cb <- function(widget, window) {
  dialog <- gtkFileChooserDialog("working directory", window, "select-folder","gtk-cancel", GtkResponseType["cancel"], "gtk-open",GtkResponseType["accept"])
...
open_cbFile <- function(widget, window) {
  dialog <- gtkFileChooserDialog("File", window, "open","gtk-cancel", GtkResponseType["cancel"], "gtk-open",GtkResponseType["accept"])
...
makeMainWindowsContent<-function(main_window){
  ...
  labelStopWords <- gtkLabelNew("Stop words: ") ; gtkMiscSetAlignment(labelStopWords, xalign=1, yalign=0.5)
  ...
  labelTrunc <- gtkLabelNew("Max. num. words: ") ; gtkMiscSetAlignment(labelTrunc, xalign=1, yalign=0.5)
  ...
  blocDESC<-{
    ...
    labelHisto <- gtkLabelNew("Histogram (number of words): ") ; gtkMiscSetAlignment(labelHisto, xalign=1, yalign=0.5)
    ...
    labelBarplot <- gtkLabelNew("Barplot (number of words): ") ; gtkMiscSetAlignment(labelBarplot, xalign=1, yalign=0.5)
    ...
    labelOccurr <- gtkLabelNew("Words occurrence: \n(may take some time)") ; gtkMiscSetAlignment(labelOccurr, xalign=1, yalign=0.5)
  ...
  blocWORD<-{
    ...
    labelMinFreq <- gtkLabelNew("Min. word occurrence:") ; gtkMiscSetAlignment(labelMinFreq, xalign=1, yalign=0.5)
    ...
    labelMaxWords <- gtkLabelNew("Max. number of words:") ; gtkMiscSetAlignment(labelMaxWords, xalign=1, yalign=0.5)
    choicesMaxWords <- rGtkDataFrame(c("Inf",seq(from=10,to=100,by=10),seq(from=200,to=500,by=100),1000))
    ...
    labelRandOrder <- gtkLabelNew("Random order:") ; gtkMiscSetAlignment(labelRandOrder, xalign=1, yalign=0.5)
    ...
    labelCol <- gtkLabelNew("Colors:") ; gtkMiscSetAlignment(labelCol, xalign=1, yalign=0.5)
    choicesCol <- rGtkDataFrame(seq(from=3,to=8,by=1))
    choicesPal <- rGtkDataFrame(c("Blues","BuGn","BuPu","GnBu","Greens","Greys","Oranges","OrRd","PuBu","PuBuGn","PuRd","Purples","RdPu","Reds","YlGn","YlGnBu","YlOrBr","YlOrRd","BrBG","PiYG","PRGn","PuOr","RdBu","RdGy","RdYlBu","RdYlGn","Spectral","Accent","Dark2","Paired","Pastel1","Pastel2","Set1","Set2","Set3"))
    ...
    labelOutFile <- gtkLabelNew("Output file format:") ; gtkMiscSetAlignment(labelOutFile, xalign=1, yalign=0.5)
    choicesOutFile <- rGtkDataFrame(c("png"))
  ...
  gSignalConnect(buttonOK_WC, "clicked", function(widget,wordFreq=mergedD) {
    ...
    makeWordcloud(wordF=wordFreq,wcFormat=myOutFile,wcminFreq=myMinFreq,wcmaxWords=myMaxWords,wcRandOrder=myRandOrder,wcCol=RColorBrewer::brewer.pal(myCol,myPal),getPlot=c(doAllWc,doOneWc))
